...
function ShelterComment() {
    ...
    useEffect(() => {
        ...
        loadData();
    }, [shelter_id, reload, url, navigate]);
    ...
    async function prevPage() {
        ...
        try {
            ...
            // If error code is 404, redirect to unauthorized page??
            // If error code is 401, redirect to not found page??
            if (error.response.status === 404) {
                navigate("/404");
            } else if (error.response.status === 401) {
                navigate("/401");
...
function ApplicationComment() {
    ...
    useEffect(() => {
        async function loadData() {
            try {
                const accessToken = localStorage.getItem('access_token');
                axios.defaults.headers.common['Authorization'] = "Bearer " + accessToken;
                const response = await axios.get(`${url}comments/app/` + application_id);
                const response2 = await axios.get(`${url}applications/` + application_id);
        ...
        loadData();
    }, [application_id, reload, url, navigate]);
    ...
    function sendRequest() {
        ...
        if (replyTarget === null) {
            axios.post(`${url}comments/app/${application_id}/`, data)
            .then(() => {
                document.getElementById("comment-box").value = "";
                setReload(!reload);
            }).catch((error) => {
            ...
            axios.post(`${url}comments/app/${application_id}/${replyTarget.id}/`, data)
            .then(() => {
                document.getElementById("comment-box").value = "";
                setReplyTarget(null);
                setReload(!reload);
    ...
    function reportComment(comment_id) {
        // redirect to report form
        navigate(`/report/application/${application_id}/${comment_id}`);
    }
    ...
    async function prevPage() {
        ...
        try {
            const response = await axios.get(`${url}comments/app/` + application_id + `/?page=${page.current}`);
            nextPageExists.current = response.data.next;
            let all_comments = response.data.results;
            for (let i = 0; i < all_comments.length; i++) {
                const response3 = await axios.get(`${url}accounts/comments/` + all_comments[i].commenter);
...
export { ShelterComment, ApplicationComment };